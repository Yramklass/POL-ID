...
# --- Step 1: Load the OLD model to extract class names ---
OLD_MODEL_PATH = '/scratch/rmkyas002/pollen_parallel_fusion_final_full.pth'
print(f"Temporarily loading old model from {OLD_MODEL_PATH} to get class names...")

# Load the full model object just for its attributes
old_model_for_names = torch.load(OLD_MODEL_PATH, map_location=DEVICE, weights_only=False) 
# Extract the class names
class_names = old_model_for_names.class_names
num_classes = len(class_names)
print(f"Successfully extracted {num_classes} class names. Discarding old model.")
# The old_model_for_names variable can now be garbage collected

class_names = old_model_for_names.class_names
num_classes = len(class_names)

# --- Step 2: Create the NEW model and load its weights ---
NEW_CHECKPOINT_PATH = '/scratch/rmkyas002/checkpoint.pt'
print(f"Loading new model weights from {NEW_CHECKPOINT_PATH}...")

# Create a new instance of your model architecture WITHOUT the dropout_rate argument
model = ParallelFusionModel(num_classes=num_classes) 

# Load the saved weights (the state_dict) from the .pt file
model.load_state_dict(torch.load(NEW_CHECKPOINT_PATH, map_location=DEVICE))

# --- Step 3: Prepare the final model for inference ---
model.eval()
model.to(DEVICE)
model.class_names = class_names 

print(f"New model is ready for inference.")
